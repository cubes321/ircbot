...
def on_message(connection, event):
    ...
    chan = event.target
    logging(event, inputtext)
    if event.arguments[0].find(NICK.lower()) != -1:
        inputtext = event.arguments[0].replace(NICK.lower(),"").strip()
        inputtext = event.source.nick + ": " + inputtext
        print(inputtext)
        get_ai_answer(inputtext, connection, event)
        return
    if event.arguments[0].find("clarkson") != -1:
        inputtext = event.source.nick + ": " + inputtext
        print(inputtext)
        get_ai_answer(inputtext, connection, event)
        return
    if event.arguments[0].find("hammond") != -1:
        inputtext = event.source.nick + ": " + inputtext
        print(inputtext)
        get_ai_answer(inputtext, connection, event)
        return
